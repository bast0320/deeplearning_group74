...
print("Hi! Let's get started with training.")
import torch
import torch
...
if __name__ == "__main__":
    ...
    dm = QM9DataModule(target=6, subset_size= 30000,  batch_size_train=batch_size_input, batch_size_inference=batch_size_input, seed = 4)
    dm.prepare_data()
    dm.setup()
    cutoff = 5
    ...
    num_epochs = 150
    test = "SWA" # Test = True is for loading a saved model and evaluating it on the test set. 
    # test = False is for training a new model.

    save_model = True
    save_val_train_losses = True
    ...
    with Profiler(interval=0.1) as profiler:
        ...
        if test == "SWA":
            ...
            # optimizer = torch.optim.SGD(model.parameters(), lr=0.0001) # , could be, but we use AdamW for this... 
            optimizer = optim.AdamW(model.parameters(), lr=5*1e-3, weight_decay=0.01)
            swa_model = torch.optim.swa_utils.AveragedModel(model)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.95) # step size is per epoch...  gamma is multiplicative...
            swa_start = 100
            end_update_learning_rate = num_epochs - 3
            swa_constant_learning_rate = torch.optim.lr_scheduler.LinearLR(optimizer, 1, 1, -1)
            # swa_scheduler = torch.optim.swa_utils.SWALR(optimizer, swa_lr=0.005) # to be used when SWA_LR = const.
            swa_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma = 0.7) # to be used when SWA_LR = const.
            ...
            for epoch in range(num_epochs):
                ...
                for batch, i in zip(train_loader, range(int(len(train_loader)))):
                    print("Epoch: ", epoch, "Batch: ", i, "LR: ", optimizer.param_groups[0]['lr'])
                    # if any(batch.y > 3.5):
                    #     print("Batch: ", i, "contains a target > 3.5")
                    #     print("Target: ", batch.y)
                    #     continue
                    ...
                    num_nodes = r_ij.shape[0]
                    # print the value of model parameters
                    ...
                    # print("train losses, last 5: ", train_losses[-5:-1] )
                ...
                if epoch > swa_start and epoch < end_update_learning_rate:
                    swa_model.update_parameters(model)
                    swa_scheduler.step()
                elif epoch >= end_update_learning_rate:
                    swa_model.update_parameters(model)
                    swa_constant_learning_rate.step()
                else: # should only change the learning rate...
                    scheduler.step()
                
            # save val_losses and train_losses as pickle
            # create a string with the name of the file + current 
            if save_val_train_losses == True:
                name_val = "val_losses_SWA" + str(datetime.datetime.now()) + ".pkl"
                name_train = "train_losses_SWA" + str(datetime.datetime.now()) + ".pkl"
            ...
            # save the swa model
            if save_model == True:
                model_name = "model_SWA" + str(datetime.datetime.now()) + ".pt"
            ...
            with torch.no_grad():
                for batch, i in zip(test_loader, range(len(test_loader))):
                    ...
                    row, col = edge_index
                    edge_attr = r_ij[row] - r_ij[col]
                    output_lengthOfr_ij_SWA = torch.sum(swa_model(dm.target, s0, v0, edge_index, edge_attr), dim = 1)
                    targets = torch.squeeze(batch.y)
                    output = torch.zeros_like(targets, dtype=torch.float32)
                    output.index_add_(0, batch.batch, output_lengthOfr_ij_SWA)

                    # converting to correct units, eV -> meV if applicable
                    output = dm.unit_conversion[dm.target](output)
                    targets = dm.unit_conversion[dm.target](targets)

                    loss_SWA = loss_fn(output, targets)
                    if i == 1:
                        print("Output: ", output, "Target: ", targets, "Error: ", loss.item())
                        
                    # test_loss += loss.item()
                    test_losses_SWA.append(loss_SWA.item())
            ...
            name_test = "test_losses_big_SWA" + str(datetime.datetime.now()) + ".pkl"
            ...
            plt.plot(test_losses_SWA, label="Test loss in MAE SWA")
            plt.legend()
            plt.title("Test loss (MAE) for PaiNN. Target: " + dictionary_targets[dm.target])
            plot_name = "plot_train_val_test" + str(datetime.datetime.now()) + ".pdf"
        ...
        if test == False:
            ...
            for epoch in range(num_epochs):
                startTimeBatch = datetime.datetime.now()
                step = 0
                ...
                for batch, i in zip(train_loader, range(int(len(train_loader)))):
                    ...
                    if step % validation_every_steps == 0:
                        ...
                        print("Step: ", step, "Validation loss mean of last 50: ", np.mean(val_losses[-50:-1]))
                endTimeBatch = datetime.datetime.now()
                print("Time for epoch: ", endTimeBatch - startTimeBatch)
            # save val_losses and train_losses as pickle
            ...
            # save the model
            model_name = "model_fulldata_gpu_subset30k" +str(test) + "_" + str(datetime.datetime.now()) + ".pt"
            ...
            plt.plot(val_losses, label="Validation loss")
            plt.yscale('log')  # Set y-axis to log scale
            plot_name = "plot_train_val" + str(datetime.datetime.now()) + ".pdf"
            plt.savefig(plot_name)
            plt.close()
            ...
            name_test = "test_losses_gpu" + str(datetime.datetime.now()) + ".pkl"
            with open(name_test, "wb") as f:
                pickle.dump(test_losses, f)
            print("Test loss MAE average: ", np.mean(test_losses))
            plt.figure(figsize=(10,5))
            plt.plot(test_losses, label="Test loss in MAE")
            plt.title("Test loss (MAE) for PaiNN. Target: " + dictionary_targets[dm.target])
            plot_name = "plot_test" + str(datetime.datetime.now()) + ".pdf"
            plt.savefig(plot_name)
