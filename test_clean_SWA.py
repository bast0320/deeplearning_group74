...
if __name__ == "__main__":
    ...
    batch_size_input = 20
    dm = QM9DataModule(target=3, subset_size= 1000, batch_size_train=batch_size_input, batch_size_inference=batch_size_input, seed = 3)
    dm.prepare_data()
    dm.setup()
    cutoff = 5
    ...
    with Profiler(interval=0.1) as profiler:
        ...
        if test == "SWA":
            ...
            optimizer = optim.AdamW(model.parameters(), lr=5*1e-3, weight_decay=0.05)
            swa_model = torch.optim.swa_utils.AveragedModel(model)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.85) # step size is per epoch...  gamma is multiplicative...
            swa_start = 10
            ...
            swa_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma = 0.7) # to be used when SWA_LR = const.
            ...
            if save_val_train_losses == True:
                name_val = "val_losses_subset10k_SWA" + str(datetime.datetime.now()) + ".pkl"
                name_train = "train_losses_subset10k_SWA" + str(datetime.datetime.now()) + ".pkl"
            ...
            # save the swa model
            if save_model == True:
                model_name = "model_sub10k_SWA" + str(datetime.datetime.now()) + ".pt"
